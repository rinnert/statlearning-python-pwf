...
\begin{frame}{Prediction}
	...
	\end{itemize}
	\bottomline{Here $\bm{\hat{f}}$ is treated as a \e{black box}. But we don't have to!}
\end{frame}
...
\begin{frame}{Prediction Accuracy}
	\begin{itemize}
		...
		\item The average $E[x]$ is often written as $\overline{x}$ or $\langle x \rangle$.
		\item The variance is then $\text{Var}(x) = \langle (x - \overline{x})^2 \rangle
			= \langle x^2 \rangle - {\langle x \rangle}^2 = \overline{x^2} - \overline{x}^2$.
	\end{itemize}
